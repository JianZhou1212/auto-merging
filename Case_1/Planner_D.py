...
class Planner_D( ):
    ...
    def ReachableSet(self, last_ax_SV, current_x_SV, current_y_SV):
        N = self.N
        N_Car = self.N_Car
        l_veh = self.l_veh
        w_veh = self.w_veh
        ...
        for i in range(N_Car - 1):
            ...
            x_before = current_x_SV[i]
            for t in range(1, N + 1):
                x_next = A_SV@x_before + B_SV*last_ax_SV
                min_x = x_next[0] - l_veh
                max_x = x_next[0] + l_veh
    ...
    def Return(self, last_ax_SV, current_x_SV, current_y_SV, current_x_EV):
        N_M = self.N_M
        T   = self.T
        N   = self.N
        L_Center = self.L_Center
        ...
        G, g, Robust_SV_Position_k = self.ReachableSet(last_ax_SV, current_x_SV, current_y_SV)
