...
class Planner_R( ):
    def __init__(self, Params):
        ...
        self.low_bound_ax = Params['low_bound_ax']
        self.up_bound_ax  = Params['up_bound_ax']
    ...
    def ReachableSet(self, current_x_SV):
        ...
        low_bound_ax = self.low_bound_ax
        up_bound_ax  = self.up_bound_ax
        
        BU_SV_Poly = Polytope(np.array([[B_SV_coarse[0]*low_bound_ax, B_SV_coarse[1]*low_bound_ax], [B_SV_coarse[0]*up_bound_ax, B_SV_coarse[1]*up_bound_ax]])) # line segmenr in R^2
        Reachable_Set = list( )
        ...
        for i in range(N_Car - 1):   
            ...
            G_i = np.zeros((4, 2*N))
            g_i = np.zeros((4, N))
            Reachable_Set_i = list( )
            Reachable_Set_i.append(current_x_SV[i])
            for t in range(1, N_coarse + 1):
                if t == 1:
                    reachable_set_t         = (A_SV_coarse@Reachable_Set_i[t - 1] + BU_SV_Poly) 
                else:
                    reachable_set_t         = (A_SV_coarse*Reachable_Set_i[t - 1] + BU_SV_Poly) & X_SV_Poly
                Reachable_Set_i.append(reachable_set_t)
            ...
            Reachable_Set.append(Reachable_Set_i)
            G.append(G_i)
            g.append(g_i)
    ...
    def Return(self, current_x_SV, current_y_SV, current_x_EV):
        N_M = self.N_M
        T   = self.T
        N   = self.N
        L_Center = self.L_Center
        ...
        G, g, Robust_SV_Position_k = self.ReachableSet(current_x_SV)
        REF = self.ProjectSpeed(Robust_SV_Position_k, current_x_SV, EV_point_mass_k, SpeedNom) 
